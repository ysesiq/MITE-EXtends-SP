...
public class TileEntityFurnaceTrans extends TileEntity {
   ...
   public static int getHeatLevelRequired(int item_id) {
         // 高燃燃料=振金，艾德曼
      if (item_id == Block.coalBlock.blockID || item_id == Blocks.netherAdamantiumOre.blockID || item_id == Blocks.blockVibranium.blockID || item_id == Block.blockAdamantium.blockID || item_id == Items.enchantSRC.itemID) {
         return 5;
         // 烈焰棒=秘银块，远古金属块
      } else if (item_id == Block.oreAdamantium.blockID || item_id == Blocks.fancyRed.blockID || item_id == Block.blockMithril.blockID || item_id == Block.blockAncientMetal.blockID) {
         return 4;
         // 岩浆=铁块，金块，银块，铜块
      } else if (item_id == Block.oreMithril.blockID || item_id == Blocks.volcanoMithrilOre.blockID || item_id == Block.blockIron.blockID || item_id == Block.blockCopper.blockID || item_id == Block.blockSilver.blockID || item_id == Block.blockGold.blockID) {
         return 3;
         ...
         return 2;
      } else if (item_id == Block.oreNetherQuartz.blockID || item_id == Block.oreEmerald.blockID || item_id == Block.oreDiamond.blockID || item_id == Blocks.volcanoDiamondOre.blockID || item_id == Blocks.volcanoEmeraldOre.blockID || item_id == Block.oreRedstone.blockID) {
         return 2;
   ...
   public int d(int par1) {
      if (this.getFurnaceBlock().blockMaterial == Materials.vibranium) {
         return this.furnaceItemStacks[0] == null ? this.furnaceCookTime * par1 / 200 : this.furnaceCookTime * par1 / (this.furnaceItemStacks[0].getItem().getCookTime() / Math.max(this.heat_level, 1)) / 3;
      }
