...
import net.xiaoyu233.mitemod.miteite.entity.EntityUltimateAnnihilationSkeleton;
...
public class BlockMobSpawnerTrans extends BlockContainer {
    ...
    public int dropBlockAsEntityItem(BlockBreakInfo info) {
        ...
        if(info.world.isTheNether() && info.world.rand.nextFloat() < 100) {
            EntityUltimateAnnihilationSkeleton UltimateAnnihilationSkeleton = new EntityUltimateAnnihilationSkeleton(info.world);
            UltimateAnnihilationSkeleton.setPosition(info.x, info.y, info.z);
            UltimateAnnihilationSkeleton.refreshDespawnCounter(-9600);
            if(info.getResponsiblePlayer() != null) {
                UltimateAnnihilationSkeleton.setAttackTarget(info.getResponsiblePlayer());
            }
            UltimateAnnihilationSkeleton.entityFX(EnumEntityFX.summoned);
            UltimateAnnihilationSkeleton.onSpawnWithEgg(null);
            info.world.spawnEntityInWorld(UltimateAnnihilationSkeleton);
        }
        return 0;
    }
