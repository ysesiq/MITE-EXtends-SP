...
public class MonsterUtil {
    public static void addDefaultArmor(int day_count, EntityInsentient monster, boolean haveAll) {
       ...
       if (rand.nextInt(3 - Math.min(day_count / 64, 2)) == 0 || day_count > 128 || haveAll) {
          for(int index = 4; index > 0; --index) {
             if (rand.nextInt(5 - Math.min(day_count / 16, 4)) == 0 || day_count > 96 || haveAll) {
                monster.setCurrentItemOrArmor(index, (new ItemStack(Constant.ARMORS[index - 1][Math.min(getRandomItemTier(rand, day_count) + (day_count > 365 ? 1 : 0), Constant.ARMORS[index - 1].length - 1)])).randomizeForMob(monster, day_count > 16));
             }
          }
       }
    }
